...
class ResnetBlockDDPM(nn.Module):
  ...
  def __init__(self, act, in_ch, out_ch=None, temb_dim=None, conv_shortcut=False, dropout=0.1):
    ...
    self.act = act
    self.Conv_0 = ddpm_conv3x3_pos_kernel(in_ch, out_ch)
    ...
    self.Conv_1 = ddpm_conv3x3_pos_kernel(out_ch, out_ch, init_scale=0.)
    if in_ch != out_ch:
      if conv_shortcut:
        self.Conv_2 = ddpm_conv3x3_pos_kernel(in_ch, out_ch)
      else:
  ...
  def forward(self, x, temb=None):
    ...
    if C != out_ch:
      if self.conv_shortcut:
        ...
        x = self.NIN_0(x)
    # print(f"x = {x.max().item(), x.min().item(), x.mean().item(), x.std().item()}")
    # print(f"h = {h.max().item(), h.min().item(), h.mean().item(), h.std().item()}")

    return x + h
  

##########################################################################################
#     The Followings are modification made by Sujong Lee / lees0196
#     Implementation of Partial Input-Convex Neural Network with ResNet Architecture
#
#
#
##########################################################################################

class Conv2d_pos_kernel(nn.Conv2d):
    def forward(self, input):
        # Apply softplus to the weight
        weight_transformed = torch.clamp(self.weight, min=0)
        # Perform convolution with the transformed weights
        return F.conv2d(
            input, weight_transformed, self.bias, self.stride,
            self.padding, self.dilation, self.groups
        )


def ddpm_conv1x1_pos_kernel(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1., padding=1):
  """1x1 convolution with DDPM initialization."""
  conv = Conv2d_pos_kernel(in_planes, out_planes, kernel_size=1, stride=stride, padding=padding,
                   dilation=dilation, bias=bias)
  conv.weight.data = default_init(init_scale)(conv.weight.data.shape)
  nn.init.zeros_(conv.bias)
  return conv
...
def ddpm_conv3x3_pos_kernel(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1., padding=1):
  """3x3 convolution with DDPM initialization."""
  conv = Conv2d_pos_kernel(in_planes, out_planes, kernel_size=3, stride=stride, padding=padding,
                   dilation=dilation, bias=bias)
  conv.weight.data = default_init(init_scale)(conv.weight.data.shape)
  nn.init.zeros_(conv.bias)
  return conv
...
class Downsample_pos_kernel(nn.Module):
  def __init__(self, channels, with_conv=False):
    super().__init__()
    if with_conv:
      self.Conv_0 = ddpm_conv3x3_pos_kernel(channels, channels, stride=2, padding=0)
    self.with_conv = with_conv
  ...
  def forward(self, x):
    ...
    assert x.shape == (B, C, H // 2, W // 2)
    # print(f"Changed shape ({B}, {C}, {H}, {W}) -> ({B}, {C}, {H//2}, {W//2})")
    x = F.group_norm(x, num_groups=min(32, C), weight=None, bias=None)
...
class Linear_pos_kernel(nn.Module):
    ...
    def forward(self, x):
        weight = F.softplus(self.weight_raw)  # Ensures weights are positive
        return F.linear(x, weight, self.bias)



class Upsample_pos_kernel(nn.Module):
  def __init__(self, channels, with_conv=False):
    super().__init__()
    if with_conv:
      self.Conv_0 = ddpm_conv3x3_pos_kernel(channels, channels)
    self.with_conv = with_conv
...
def get_fourier_embedding(t, embedding_dim: int, scale=1.0):
    """
    ...
    # Sample frequency coefficients b_j ~ N(0, scale^2)
    b = torch.randn(embedding_dim, device=t.device) * scale
    
    # Ensure t has shape (N, 1)
    t = t.float().view(-1, 1)
    
    # Compute 2π * t * b (each element in b multiplies the entire time vector)
    projection = 2 * math.pi * t * b[None, :]  # shape: (N, half_dim)
    
    # Apply sin and cos
    # sin_emb = torch.sin(projection)
    sin_emb = projection
    # cos_emb = torch.cos(projection)
    cos_emb = projection
...
class PICNN_ResnetBlockDDPM(nn.Module):
  """
  ...
  def __init__(self, act, in_ch, resolution, out_ch=None, data_ch=1, dropout=0.1, temb_dim=64, conv_shortcut=True):
    super().__init__()
    if out_ch is None:
      out_ch = in_ch
    # print(f"input channel = {in_ch}, output channel = {out_ch}")

    self.GroupNorm = nn.GroupNorm(num_groups=32, num_channels=in_ch, eps=1e-6)


    self.act = act
    self.relu = nn.ReLU()
    
    self.Conv_x = ddpm_conv3x3(data_ch, out_ch)
    self.Conv_z = ddpm_conv3x3_pos_kernel(in_ch, out_ch)

    self.temb_dim = temb_dim

    self.Wzu = nn.Linear(2*temb_dim, in_ch * resolution * resolution, bias = True)
    self.Wxu = nn.Linear(2*temb_dim, data_ch * resolution * resolution, bias = True)
    self.W = nn.Linear(2*temb_dim, resolution * resolution, bias = True)
    self.Wu = nn.Linear(2*temb_dim, out_ch * resolution * resolution, bias = True)

    self.unflatten_zu = nn.Unflatten(1, (in_ch, resolution, resolution))
    self.unflatten_u = nn.Unflatten(1, (out_ch, resolution, resolution))
    self.unflatten_xu = nn.Unflatten(1, (data_ch, resolution, resolution))
  ...
  def forward(self, z, x, u):
    ...
    """
    # nz = self.GroupNorm(z)
    z = self.act(F.group_norm(z, num_groups=min(32, z.size(1)), weight=None, bias=None, eps=1e-6))
    u = self.act(F.group_norm(u, num_groups=min(32, u.size(1)), weight=None, bias=None, eps=1e-6))
    
    u_comp = self.unflatten_u(self.Wu(u))

    # self_Wzu_max_weight = self.Wzu.weight.max()
    # self_Wzu_min_weight = self.Wzu.weight.min()
    # print(f"self.Wzu Max: {self_Wzu_max_weight.item()}, self.Wzu Min: {self_Wzu_min_weight.item()}")

    zu = self.relu(self.Wzu(u)) # After Wzt, the dimension now is resolution * resolution x 1
    zu = self.unflatten_zu(zu) # has resolution x resolution dimension
    ...
    z_op = zu * z
    z_comp = self.Conv_z(z_op)

    zx = self.Wxu(u) # After Wzt, the dimension now is resolution * resolution x 1
    zx = self.unflatten_xu(zx) # has resolution x resolution dimension

    x_op = zx * x
    x_comp = self.Conv_x(x_op)
    ...
    if C != self.out_ch:
      if self.conv_shortcut:
        ...
        z = self.NIN_0(z)
    # print(f"z = {z.max().item(), z.min().item(), z.mean().item(), z.std().item()}")
    # print(f"u = {u.max().item(), u.min().item(), u.mean().item(), u.std().item()}")
    # print(f"u_comp = {u_comp.max().item(), u_comp.min().item(), u_comp.mean().item(), u_comp.std().item()}")
    # print(f"x = {x.max().item(), x.min().item(), x.mean().item(), x.std().item()}")
    # print(f"x_comp = {x_comp.max().item(), x_comp.min().item(), x_comp.mean().item(), x_comp.std().item()}")
    # print(f"z_comp = {z_comp.max().item(), z_comp.min().item(), z_comp.mean().item(), z_comp.std().item()}")
    z_update = self.act(z_comp + x_comp + u_comp) + z
    # print(f"z_update = {z_update.max().item(), z_update.min().item(), z_update.mean().item(), z_update.std().item()}")
    return z_update
